...
The SingleCellExperiment object and all other Robjects needed to recreate this document are stored in the Box; the [BiocFileCache package](https://bioconductor.org/packages/release/bioc/html/BiocFileCache.html) must be installed to make use of that.

```{r load_SCE, echo=TRUE, message=TRUE}
fls <- fread("data/data_links.txt", header=TRUE)
sln <- load_RDSdata_from_Box(shared_link = fls[object == "SCE"]$link)
sln
```
```{r define_colors}
## define color scheme
...
load_data_from_Box(fls[object == "tscan"]$link, load_rds = FALSE)
```
...
for(GN in c("Tcf7","Sell","Entpd1","Pdcd1","Lef1")){
    tt <- scABC2::make_long_dt(sln, features = GN,
        exprs_values = "logcounts",
...
mks <- scABC2::extract_markers(sln, 
...
##openxlsx::write.xlsx(clust.markers.df, file = "~/Documents/Projects/2019-03_Sofia_SchietingerLab/CITEseq/data/NNEDS TO BE DETERMINED",rowNames = TRUE)
```
...
mks.up <- scABC2::extract_markers(sln, 
...
```{r eval=FALSE}
clust.markers.up.df <- lapply(clust.markers.up, as.data.frame)
openxlsx::write.xlsx(clust.markers.up.df, file = "~/Documents/Projects/2019-03_Sofia_SchietingerLab/CITEseq/data/cluster_genes_UP_panc-and-pLN_2021-04-12.xlsx",rowNames = TRUE)
```

```{r results='asis',eval=FALSE}
mks.up[classify == "unique"] %>% ABCutilities::my_table(.)
```
